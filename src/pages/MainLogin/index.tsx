...
const MainLogin = () => {
  ...
  // Check if there's a project in URL and store it
  useEffect(() => {
    const pathSegments = location.pathname.split('/').filter(Boolean);
    const projectFromUrl = pathSegments[0];
    
    if (projectFromUrl && ['cdu', 'rekyc', 'ddpi', 'account-closures'].includes(projectFromUrl)) {
      sessionStorage.setItem('targetProject', projectFromUrl);
    }
  }, [location.pathname]);
